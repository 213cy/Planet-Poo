...
myPin
:   1×2的向量,myPin标记您控制的腹泻生物在地图(map)中的位置.
...
oppsPin
:   k×2的矩阵,oppsPin在同一张地图下的其它生物（竞争的腹泻生物）的位置.
...
####5.	该函数的其他说明
 1. 该函数不限长度,不限执行时间（执行时间不要太过分）
 2. 可以使用Matlab任何工具箱的函数
 3. 而且允许外部调用
...
## 背景说明
 ...
 Matlab一个叫“[Mars Surveyor](http://www.mathworks.cn/matlabcentral/contest/contests/4/rules)”的编程竞赛

 这里有一个名叫 "[tron battle](http://www.codingame.com/multiplayer)" 的游戏.这个帖子和这个游戏是一样的 ,不过不能用matlab编程要写的代码可能会更多.

 如果该版本的程序在您低版本的MATLAB中运行时报错,您可以尝试使用[低版本的游戏代码](http://www.ilovematlab.cn/thread-285238-1-1.html)
